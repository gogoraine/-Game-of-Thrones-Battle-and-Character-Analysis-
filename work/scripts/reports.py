...
def generate_simple_report(df_characters):
    ...
    try:
        ...
        final_df = filtered_df[["id", "имя", "дом",
                                "культура", "пол", "возраст"]]
...
def generate_stat_report(df_characters):
    """
    Вычисляет статистические показатели для переменных.

    Описание:
        Вычисляет статистические показатели
        для качественных переменных (частоты, доли)
        и количественных признаков (минимум, максимум, среднее, дисперсия,
        стандартное отклонение возраста)
        с фиксацией результатов в текстовом файле.
    Входные параметры:
    ...
    try:
        ...
        with (open(report_path, "w", encoding="utf-8") as f):
            ...
            for col in ["жив/мертв", "пол"]:
                ...
                percentages = df_characters[col]
                percentages = percentages.value_counts(normalize=True) * 100
                for idx in counts.index:
                    f.write(f"  {idx:<10} | Количество: "
                            f"{counts[idx]:<5} | Доля: "
                            f"{percentages[idx]:.2f}%\n")
                f.write("\n")
...
def generate_pivot_report(df_characters):
    ...
    Описание:
        Строит двухмерную сводную таблицу
        взаимного распределения полов персонажей
        по топ-10 наиболее часто встречающимся
        Великим Домам и сохраняет её в файл.
    Входные параметры:
        df_characters (pd.DataFrame)
        - таблица с данными персонажей.
    Возвращаемый объект: Нет.
    ...
    try:
        ...
        top_houses = df_characters["дом"]\
            .value_counts().head(10).index.tolist()
        sub = df_characters["дом"].isin(top_houses)
        df_filtered = df_characters[sub]
...
def generate_clustered_bar_chart(df_characters):
    """
    Создаёт кластеризованную столбчатую диаграмму.

    Описание:
        Создает и сохраняет кластеризованную столбчатую диаграмму,
        отражающую
        соотношение живых и погибших членов
        для топ-5 Домов и сводной группы.
    Входные параметры:
        df_characters (pd.DataFrame)
        - таблица с данными персонажей.
    Возвращаемый объект: Нет.
    ...
    try:
        ...
        homs = df_copy["дом"]
        df_copy["дом_группа"] = homs.apply(lambda x:
                                           x if x in top_houses
                                           else "Другие Великие Дома")
        ...
        ax.set_title("Соотношение живых и погибших "
                     "персонажей по Великим Домам",
                     fontsize=14,
                     fontweight="bold",
                     pad=15)
...
def generate_categorized_histogram(df_characters):
    ...
    Описание:
        Строит гистограмму распределения возраста персонажей,
        категоризированную
        по половому признаку с помощью
        наложения полупрозрачных слоев частот.
    Входные параметры:
        df_characters (pd.DataFrame)
        - таблица с данными персонажей.
    Возвращаемый объект: Нет.
    ...
    try:
        ...
        # Гистограмма женщин
        ax.hist(female_ages, bins=20,
                alpha=0.6,
                color="#e74c3c",
                label="Женщины",
                edgecolor="black")

        ax.set_title("Категоризированная "
        ...
        plt.tight_layout()
        img_path = os.path.join(graphics_dir,
                                "2_categorized_histogram.png")
...
def generate_boxplot(df_characters):
    ...
    Описание:
        ...
        возраста персонажей по топ-5 Великим Домам
        путем построения
        одномерных диаграмм Бокса-Вискера (ящиков с усами).
    Входные параметры:
        df_characters (pd.DataFrame)
         - таблица с данными персонажей.
    Возвращаемый объект: Нет.
    ...
    try:
        ...
        data_to_plot = [df_copy[df_copy["дом"] == house]["возраст"]
        ...
        ax.set_title("Диаграмма Бокса-Вискера:"
                     " Распределение возраста по Домам",
                     fontsize=14,
                     fontweight="bold",
                     pad=15)
        ax.set_xlabel("Великие Дома Вестероса", fontsize=12)
...
def generate_scatter_plot(df_battles):
    """
    Строит диаграмму рассеивания сражений по годам.

    Описание:
        Строит двухмерную диаграмму рассеивания
        хронологической последовательности
        сражений (год проведения относительно ID записи)
         с цветовым кодированием регионов.
    Входные параметры:
    ...
    try:
        ...
        ax.set_title("Диаграмма рассеивания сражений по годам и регионам",
                     fontsize=14,
                     fontweight="bold",
                     pad=15)
        ...
        ax.legend(title="Регионы Вестероса",
